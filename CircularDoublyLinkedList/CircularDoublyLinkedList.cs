...
namespace CircularDoublyLinkedList
{
    public class CircularDoublyLinkedList<T> : IEnumerable<T>  // кольцевой двусвязный список
    {
        ...
        public void Add(T data)
        {
            ...
            {
                //предыдущий-прдедыдущий головного
                node.Previous = head.Previous;
                //сделующий головной
                node.Next = head;
                //у предыдущего головного следующий - новый узел
                head.Previous.Next = node;
                //у головного предыдущий - новый
                head.Previous = node;
            }
            count++;
        }
        ...
        IEnumerator<T> IEnumerable<T>.GetEnumerator()
        {
            ...
            do
            {
                if (current != null)
                {
                    //yield return используется для возврата каждого элемента по одному
